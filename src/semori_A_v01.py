...
def keyword_extraction(sentences):
    ...
    for sentence in sentences:
        ...
        items_in_sentence = []
        for item,tag in pos:
            if tag in ['Noun','Verb','Adjective'] and (not item in stopwords) and (not item in items_in_sentence):
                if item in single.keys():
                    single[item] += 1
                else :
                    single[item] = 1
                items_in_sentence.append(item)
    single = sorted(single.items(),key=lambda x:x[1],reverse=True)[:10]
    ret_single = {}
    ...
    for sentence in sentences:
        ...
        noun = None
        for item,tag in pos:
            if tag == 'Noun' and (not item in stopwords):
                noun = item
            elif tag in ['Verb','Adjective']:
                if noun and (not item in stopwords):
                    tuples.append((noun,item))
                    noun = None
    double = {}
    for tuple in tuples:
        if tuple in double.keys():
            double[tuple] += 1
        else :
            double[tuple] = 1
    double =  sorted(double.items(),key=lambda x:x[1],reverse=True)[:10]
    ret_double = {}
...
if __name__ == "__main__":
    ...
    write = open("output/" + target + "v01/good_single.txt",'w',encoding='utf-8')
    for key in keyw_good_single.keys():
        write.write(f"{key} >> {keyw_good_single[key]}%\n")
    write.close()
    
    write = open("output/" + target + "v01/good_double.txt",'w',encoding='utf-8')
    for key in keyw_good_double.keys():
        write.write(f"{key} >> {keyw_good_double[key]}%\n")
    write.close()
    
    write = open("output/" + target + "v01/bad_single.txt",'w',encoding='utf-8')
    for key in keyw_bad_single.keys():
        write.write(f"{key} >> {keyw_bad_single[key]}%\n")
    write.close()
    
    write = open("output/" + target + "v01/bad_double.txt",'w',encoding='utf-8')
